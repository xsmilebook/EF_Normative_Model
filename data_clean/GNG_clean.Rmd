...
# Identify outliers based on GNG_acc (beyond 3 SD from the age group mean)
outlier_data <- data_before_outliers %>%
  group_by(ageGroups) %>%
  mutate(
    mean_GNG_acc = mean(GNG_acc, na.rm = TRUE),
    sd_GNG_acc = sd(GNG_acc, na.rm = TRUE),
    is_outlier = (mean_GNG_acc - GNG_acc) > 3 * sd_GNG_acc
  ) %>%
  ungroup()

# Report removed outliers for each age group to the report file
outlier_report <- outlier_data %>%
  filter(is_outlier) %>%
  count(ageGroups, .drop = FALSE)
for (i in 1:nrow(outlier_report)) {
  report_line <- sprintf("Removed GNG_acc outliers (Age = %s): %d", outlier_report$ageGroups[i], outlier_report$n[i])
...
cat(sprintf("%-50s | Removed: %4d | Remaining: %4d\n", "Removed GNG_acc outliers (>3 SD from age group mean)", removed_outlier_count, nrow(data_after_outliers)))

# Clean up temporary columns and save the result
final_between_data <- data_after_outliers %>% select(-mean_GNG_acc, -sd_GNG_acc, -is_outlier, -ageGroups)
...
# Remove outliers based on d-prime score by age group
GonogoTable <- GonogoTable %>%
  mutate(ageGroups = cut(Age_year, breaks = binEdges, right = TRUE, include.lowest = TRUE, labels = paste(binEdges[-length(binEdges)], binEdges[-1], sep="-")))
...
final_dprime_data <- GonogoTable %>%
  ...
  mutate(
    ...
    is_outlier_dprime = (mean_d_prime - d_prime) > 3 * std_d_prime
  ) %>%
  ungroup()

# Report removed d-prime outliers to the report file
dprime_outlier_report <- final_dprime_data %>%
  filter(is_outlier_dprime) %>%
  count(ageGroups, .drop = FALSE)
for (i in 1:nrow(dprime_outlier_report)) {
  report_line <- sprintf("Removed d-prime outliers (Age = %s): %d", dprime_outlier_report$ageGroups[i], dprime_outlier_report$n[i])
  write(report_line, file = report_path, append = TRUE)
}

# Remove d-prime outliers from the final dataset
final_dprime_filtered <- final_dprime_data %>% filter(!is_outlier_dprime)

# Report total d-prime outliers removed to console
removed_dprime_count <- sum(dprime_outlier_report$n)
cat(sprintf("%-50s | Removed: %4d | Remaining: %4d\n", "Removed d-prime outliers (>3 SD from age group mean)", removed_dprime_count, nrow(final_dprime_filtered)))
