...
binEdges <- 11:18 # Make sure this is defined if it's not carried over
GonogoTable <- GonogoTable %>%
  mutate(ageGroups = cut(Age_year, breaks = binEdges, right = TRUE, include.lowest = TRUE, labels = paste(binEdges[-length(binEdges)], binEdges[-1], sep="-")))


# --- [CORRECTED] Step 4.2: Identify d-prime outliers using absolute difference ---
...
# --- [CORRECTED] Step 4.3: Generate an accurate report for d-prime outliers ---
dprime_outlier_report <- GonogoTable %>%
  group_by(ageGroups) %>%
  summarise(
    # Calculate the true mean and sd for the ENTIRE age group
    mean_for_report = mean(d_prime, na.rm = TRUE),
    sd_for_report = sd(d_prime, na.rm = TRUE),
    # Directly count the number of d-prime outliers within each group
...
# --- [CORRECTED] Step 4.4: Write the accurate d-prime report to the file ---
for (i in 1:nrow(dprime_outlier_report)) {
  report_line <- sprintf("Removed d-prime outliers (Age = %s): %d | Group Mean=%.4f, Group SD=%.4f", 
                        dprime_outlier_report$ageGroups[i], 
                        dprime_outlier_report$n_outliers[i],
                        dprime_outlier_report$mean_for_report[i],
                        dprime_outlier_report$sd_for_report[i])
...
removed_dprime_count <- sum(dprime_outlier_report$n_outliers, na.rm=TRUE)
cat(sprintf("\n%-50s | Removed: %4d | Remaining: %4d\n", 
...
# We remove the temporary 'ageGroups' column before saving
...
# We remove all temporary columns used for the calculation
