...
current_data <- report_step(current_data, current_data %>% filter(!is.na(Oneback_acc)), "Removed subjects with Oneback_acc of NA", report_path)
...
current_data <- report_step(current_data, current_data %>% filter(Oneback_acc > 0), "Removed subjects with Oneback_acc of 0", report_path)
...
# --- Step 2.1: Identify outliers based on Oneback_acc ---
...
outlier_data <- data_before_outliers %>%
  # Filter out NA groups just in case any slip through, ensuring safe calculations.
...
cat("\n--- Detailed Oneback_acc Outlier Report ---\n")
for (i in 1:nrow(outlier_summary)) {
  ...
  for (j in 1:nrow(individual_outliers)) {
    outlier_detail_line <- sprintf("    -> Removed Participant ID: %s, Oneback_acc: %.4f (Difference from mean: %.2f SDs)",
                                   individual_outliers$ID[j],
                                   individual_outliers$Oneback_acc[j],
                                   abs(individual_outliers$Oneback_acc[j] - individual_outliers$mean_acc[j]) / individual_outliers$sd_acc[j])
...
total_summary_line <- sprintf("\n%-50s | Removed: %4d | Remaining: %4d", 
                              "Total Oneback_acc outliers (>3 SD from mean)", 
                              removed_outlier_count, 
                              nrow(data_after_outliers))
cat(total_summary_line, "\n")
write(total_summary_line, file = report_path, append = TRUE)
