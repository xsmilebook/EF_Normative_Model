...
# switch <- switch %>% mutate(Age = as.numeric(Age)) # 如果需要强制转换
# 检查 Age 的范围，用于固定 x 轴
age_range <- range(switch$Age, na.rm = TRUE)
cat("Age 的范围是:", age_range[1], "到", age_range[2], "\n")
...
plot_info <- tibble::tribble(
  ...
  "SC_BP_acc", "SC: Mix - Pure Acc", "Accuracy Difference",
...
png_output_dir <- "D:/datasets/yunfu/raw_data/switch_results/figures_followup"
...
for (i in 1:nrow(plot_info)) {
  ...
  # 同时移除 IES 中可能的 Inf 值
  plot_data <- switch[is.finite(switch[[y_col]]) & is.finite(switch$Age) & 
  ...
  if (grepl("RT", y_col, ignore.case = TRUE)) {
    ...
    y_scale <- scale_y_continuous(limits = c(0.74, 0.80), expand = c(0.01, 0))
  } else {
    # 对于其他指标 (如 SC, IES, Proportion) 使用默认扩展
    y_scale <- scale_y_continuous(expand = c(0.05, 0))
  }
  
  p <- ggplot(plot_data, aes(x = Age, y = !!sym(y_col))) +
    # 使用轻微透明度的点，因为点会很多
...
# --- 5. 保存到 PDF ---
output_pdf_path <- "D:/datasets/yunfu/raw_data/switch_results/followup_switch_analysis_plots_continuous_age.pdf"
