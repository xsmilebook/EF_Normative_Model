...
x_female <- seq(min(GNG_data[[smoothvar]]), max(GNG_data[[smoothvar]]), length.out = n_points)
x_male <- seq(min(GNG_data[[smoothvar]]), max(GNG_data[[smoothvar]]), length.out = n_points)
...
eps <- 1e-07 
x.tmp0 <- seq(min(GNG_data[[smoothvar]]), max(GNG_data[[smoothvar]]), length.out = n_points)
x.tmp1 <- x.tmp0 + eps
...
# save out
if (! dir.exists(paste0(interfileFolder, "/bootstrap/GNGd_prime"))){
  dir.create(paste0(interfileFolder, "/bootstrap/GNGd_prime"))
}

saveRDS(bootstrap_centiles, paste0(interfileFolder, "/bootstrap/GNGd_prime/centile_bootstrap_", n, ".rds"))
