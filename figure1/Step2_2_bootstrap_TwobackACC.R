...
x_female <- seq(min(back2_data[[smoothvar]]), max(back2_data[[smoothvar]]), length.out = n_points)
x_male <- seq(min(back2_data[[smoothvar]]), max(back2_data[[smoothvar]]), length.out = n_points)
...
eps <- 1e-07 
x.tmp0 <- seq(min(back2_data[[smoothvar]]), max(back2_data[[smoothvar]]), length.out = n_points)
x.tmp1 <- x.tmp0 + eps
...
# save out
if (! dir.exists(paste0(interfileFolder, "/bootstrap/2backAcc"))){
  dir.create(paste0(interfileFolder, "/bootstrap/2backAcc"))
}

saveRDS(bootstrap_centiles, paste0(interfileFolder, "/bootstrap/2backAcc/centile_bootstrap_", n, ".rds"))
