...
plot(density(back1_data$Oneback_acc), main = "Density Plot of 1-back", xlab = "Value", col = "blue")
# load models
performance_1backAcc_distribution <- read.csv(paste0(resultFolder, "/performance_1backAcc_distribution.csv"))
modelsum_1backAcc_distribution <- readRDS(paste0(resultFolder, "/modelsum_1backAcc_distribution.rds"))
...
# performance bic
performance_1backAcc_distribution_adj <- performance_1backAcc_distribution %>% drop_na()
performance_1backAcc_distribution_adj <- performance_1backAcc_distribution_adj %>% 
...
#   arrange(BIC_adjusted) %>% 
#   slice(1:10)
performance_1backAcc_distribution_adj <- performance_1backAcc_distribution_adj[order(performance_1backAcc_distribution_adj$BIC, decreasing = F),]
performance_1backAcc_distribution_adj$distribution <- factor(performance_1backAcc_distribution_adj$distribution, levels=performance_1backAcc_distribution_adj$distribution)
ggplot(data = performance_1backAcc_distribution_adj, aes(x = distribution, y = BIC_adjusted)) +
  geom_bar(stat = "identity", fill = "#A4C5DF", color = "black", size = 0.5, width = 0.7) +
  labs(title = "1-back",x = "Distribution", y = expression(BIC~(x~10^4))) +  # Add a multiplier annotation to the y-axis
  theme_minimal() +
  theme(
  ...
  coord_cartesian(ylim = c(min(performance_1backAcc_distribution_adj$BIC_adjusted), max(performance_1backAcc_distribution_adj$BIC_adjusted))) + # Enforce the y-axis range
  scale_y_continuous(
    breaks = seq(-12000, -3000, by = 3000), # Set y-axis tick intervals
    labels = function(x) x / 10000, # Adjust y-axis tick labels to a simpler scientific format
    expand = c(0, 0) # Remove default padding
  )

ggsave(paste0(FigureFolder, "/1-back/distribution_BIC_1backAcc.pdf"), width=9, height=7, units = "cm")
ggsave(paste0(FigureFolder, "/1-back/distribution_BIC_1backAcc.tiff"), width=9, height=7, units = "cm")

# Remove rows with missing values and order by BIC in ascending order
performance_1backAcc_bsdf_adj<- performance_1backAcc_bsdf %>% 
  drop_na() %>%
  arrange(BIC)

# Select the 10 smallest BIC values
performance_1backAcc_bsdf_adj<- performance_1backAcc_bsdf_adj[1:10, ]

# Create a label for degrees of freedom
performance_1backAcc_bsdf_adj$bsdf <- paste0(
  "mu", performance_1backAcc_bsdf_adj$mu.df, 
  "_sigma", performance_1backAcc_bsdf_adj$sigma.df, 
  "_degree", performance_1backAcc_bsdf_adj$degree
)

# Convert bsdf to a factor with levels ordered by BIC
performance_1backAcc_bsdf_adj$bsdf <- factor(performance_1backAcc_bsdf_adj$bsdf, levels = performance_1backAcc_bsdf_adj$bsdf)

# Adjust BIC values by subtracting the maximum value
performance_1backAcc_bsdf_adj$BIC_adjusted <- performance_1backAcc_bsdf_adj$BIC - max(performance_1backAcc_bsdf_adj$BIC)

# Plot
ggplot(data = performance_1backAcc_bsdf_adj) +
  ...
  scale_y_continuous(
    breaks = seq(-24, -6, by = 6), # Define y-axis tick intervals
...
# Save the plot
ggsave(paste0(FigureFolder, "/1-back/bsdf_BIC_1backAcc.tiff"), width = 9, height = 7, units = "cm")
ggsave(paste0(FigureFolder, "/1-back/bsdf_BIC_1backAcc.pdf"), width = 9, height = 7, units = "cm")
...
summary(model.1backAcc)
mu.formula <- as.character(model.1backAcc$mu.formula)[3]
sigma.formula <- as.character(model.1backAcc$sigma.formula)[2]
print(paste0("The best distribution of gamlss for ",str_split_i(as.character(model.1backAcc$mu.formula), "~", 1)[2], " is ", model.1backAcc$family[1], "; the best mu.df is ", substr(mu.formula, regexpr("\\)", mu.formula)-1, regexpr("\\)", mu.formula)-1), ", sigma.df is ", substr(sigma.formula, regexpr("\\)", sigma.formula)-1, regexpr("\\)", mu.formula)-1), "."))
print(paste0("Diagnosis.1, model of ", str_split_i(as.character(model.1backAcc$mu.formula), "~", 1)[2], " is converged."))
...
plot(model.1backAcc, par=newpar)
# Detrended transformed Owen’s plots
dtop(model.1backAcc)
# residual plots & Q-Q plots
tiff(paste0(FigureFolder, "/1-back/diagnosis_mod_", str_split_i(as.character(model.1backAcc$mu.formula), "~", 1)[2], ".tiff"),res=600, width=4000, height=3000)
newpar <- par(mfrow = c(2, 2), mar = par("mar") + c(0, 1, 0,0),
              col.axis = "black", col = "black", col.main = "black",
              col.lab = "black", pch = 1, cex = 0.45, cex.lab = 1.2,
              cex.axis = 1, cex.main = 1.2)
...
dev.off()
# Detrended transformed Owen’s plots
tiff(paste0(FigureFolder, "/1-back/diagnosis_owenplot_mod_", str_split_i(as.character(model.1backAcc$mu.formula), "~", 1)[2], ".tiff"),res=600, width=4000, height=3000)
dtop(model.1backAcc)
dev.off()
...
```{r bootstrap}
# 1. Oneback Acc
bootstrapdir <- paste0(interfileFolder, "/bootstrap/1backAcc")
...
quantile.median$age <- seq(min(back1_data$Age_year), max(back1_data$Age_year), length.out = n_points)
quantile.P2.5$age <- seq(min(back1_data$Age_year), max(back1_data$Age_year), length.out = n_points)
quantile.P97.5$age <- seq(min(back1_data$Age_year), max(back1_data$Age_year), length.out = n_points)

# plot
ggplot()+
  geom_point(data = back1_data, aes(x = Age_year, y = Oneback_acc), color = "grey", alpha = 0.3, width = 0, height = 0.1) +
  ...
  labs(x="Age(years)", y="Acc", title="Bootstrap 95% CI of 1-back")+
  theme_classic()+
...
ggsave(paste0(FigureFolder, "/1-back/bootstrap_P50_95CI_1backAcc_all.tiff"), width=9, height=7, units = "cm")
ggsave(paste0(FigureFolder, "/1-back/bootstrap_P50_95CI_1backAcc_all.pdf"), width=9, height=7, units = "cm")
