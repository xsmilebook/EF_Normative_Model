...
EFvar <- "Oneback_acc_deviationZ"
dataname <- "oneback"
dataname0 <- "back1_data" 

# 7. data preprocessing 
back1_data[, original_vars] <- lapply(back1_data[, original_vars], as.numeric)
...
for (Time_id in Time_ids) {
  ...
  for (psyvar_arg in psyc_variables_continous_all) {
    ...
    if (!psyvar_arg %in% names(back1_data) || all(is.na(back1_data[[psyvar_arg]]))) {
