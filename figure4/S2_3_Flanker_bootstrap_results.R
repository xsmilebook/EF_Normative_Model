...
# 设置文件目录
bootstrap_dir <- "/ibmgpfs/cuizaixu_lab/tanlirou1/Yunfu/YF_EF_psy/interfileFolder/ABCD/bootstrap/Flanker"
output_dir <- "/ibmgpfs/cuizaixu_lab/tanlirou1/Yunfu/YF_EF_psy/interfileFolder/ABCD/bootstrap"
...
# 保存结果数据和图像
write.csv(derivative_summary, file = file.path(output_dir, "derivative_summary_Flanker.csv"), row.names = FALSE)
ggsave(file.path(output_dir, "mu_derivative_age_Flanker.png"), plot = p1, width = 8, height = 6, dpi = 300)
ggsave(file.path(output_dir, "sigma_derivative_age_Flanker.png"), plot = p2, width = 8, height = 6, dpi = 300)