...
Flanker_data <- read_xlsx(paste0(datapath, "/Q_1back.xlsx"))
...
plot(density(Flanker_data$nihtbx_flanker_uncorrected), main = "Density Plot of Flanker", xlab = "Value", col = "blue")
# load models
performance_Flanker_distribution <- read.csv(paste0(resultFolder, "/performance_Flanker_distribution.csv"))
modelsum_Flanker_distribution <- readRDS(paste0(resultFolder, "/modelsum_Flanker_distribution.rds"))
...
# performance bic
performance_Flanker_distribution_adj <- performance_Flanker_distribution %>% drop_na()
performance_Flanker_distribution_adj <- performance_Flanker_distribution_adj %>% 
...
#   arrange(BIC_adjusted) %>% 
#   slice(1:10)
performance_Flanker_distribution_adj <- performance_Flanker_distribution_adj[order(performance_Flanker_distribution_adj$BIC, decreasing = F),]
performance_Flanker_distribution_adj$distribution <- factor(performance_Flanker_distribution_adj$distribution, levels=performance_Flanker_distribution_adj$distribution)
ggplot(data = performance_Flanker_distribution_adj, aes(x = distribution, y = BIC_adjusted)) +
  geom_bar(stat = "identity", fill = "#A4C5DF", color = "black", size = 0.5, width = 0.7) +
  labs(title = "Flanker",x = "Distribution", y = expression(BIC~(x~10^3))) +  # Add a multiplier annotation to the y-axis
  theme_minimal() +
  theme(
  ...
  coord_cartesian(ylim = c(min(performance_Flanker_distribution_adj$BIC_adjusted), max(performance_Flanker_distribution_adj$BIC_adjusted))) + # Enforce the y-axis range
  scale_y_continuous(
    breaks = seq(-2470, -2420, by = 20), # Set y-axis tick intervals
    labels = function(x) x / 1000, # Adjust y-axis tick labels to a simpler scientific format
    expand = c(0, 0) # Remove default padding
  )

ggsave(paste0(FigureFolder, "/Flanker/distribution_BIC_Flanker.pdf"), width=9, height=7, units = "cm")
ggsave(paste0(FigureFolder, "/Flanker/distribution_BIC_Flanker.tiff"), width=9, height=7, units = "cm")

# Remove rows with missing values and order by BIC in ascending order
performance_Flanker_bsdf_adj<- performance_Flanker_bsdf %>% 
  drop_na() %>%
  arrange(BIC)

# Select the 10 smallest BIC values
performance_Flanker_bsdf_adj<- performance_Flanker_bsdf_adj[1:10, ]

# Create a label for degrees of freedom
performance_Flanker_bsdf_adj$bsdf <- paste0(
  "mu", performance_Flanker_bsdf_adj$mu.df, 
  "_sigma", performance_Flanker_bsdf_adj$sigma.df, 
  "_degree", performance_Flanker_bsdf_adj$degree
)

# Convert bsdf to a factor with levels ordered by BIC
performance_Flanker_bsdf_adj$bsdf <- factor(performance_Flanker_bsdf_adj$bsdf, levels = performance_Flanker_bsdf_adj$bsdf)

# Adjust BIC values by subtracting the maximum value
performance_Flanker_bsdf_adj$BIC_adjusted <- performance_Flanker_bsdf_adj$BIC - max(performance_Flanker_bsdf_adj$BIC)

# Plot
ggplot(data = performance_Flanker_bsdf_adj) +
  geom_bar(aes(x = bsdf, y = BIC_adjusted), stat = "identity", 
           position = "dodge", fill = "#A4C5DF", color = "black", size = 0.5) +
  labs(title = "Flanker", x = "Degree of freedom", y = expression(BIC~(x~10))) +  # Add axis labels with BIC multiplier
  theme_minimal() +
  theme(
    plot.title = element_text(size = 8.5, hjust = 0.5),
  ...
  scale_y_continuous(
    breaks = seq(-24, -4, by = 6), # Define y-axis tick intervals
...
# Save the plot
ggsave(paste0(FigureFolder, "/Flanker/bsdf_BIC_Flanker.tiff"), width = 9, height = 7, units = "cm")
ggsave(paste0(FigureFolder, "/Flanker/bsdf_BIC_Flanker.pdf"), width = 9, height = 7, units = "cm")
...
summary(model.Flanker)
mu.formula <- as.character(model.Flanker$mu.formula)[3]
sigma.formula <- as.character(model.Flanker$sigma.formula)[2]
print(paste0("The best distribution of gamlss for ",str_split_i(as.character(model.Flanker$mu.formula), "~", 1)[2], " is ", model.Flanker$family[1], "; the best mu.df is ", substr(mu.formula, regexpr("\\)", mu.formula)-1, regexpr("\\)", mu.formula)-1), ", sigma.df is ", substr(sigma.formula, regexpr("\\)", sigma.formula)-1, regexpr("\\)", mu.formula)-1), "."))
print(paste0("Diagnosis.1, model of ", str_split_i(as.character(model.Flanker$mu.formula), "~", 1)[2], " is converged."))
...
plot(model.Flanker, par=newpar)
# Detrended transformed Owen’s plots
dtop(model.Flanker)
# residual plots & Q-Q plots
tiff(paste0(FigureFolder, "/Flanker/diagnosis_mod_", str_split_i(as.character(model.Flanker$mu.formula), "~", 1)[2], ".tiff"),res=600, width=4000, height=3000)
newpar <- par(mfrow = c(2, 2), mar = par("mar") + c(0, 1, 0,0),
              col.axis = "black", col = "black", col.main = "black",
              col.lab = "black", pch = 1, cex = 0.45, cex.lab = 1.2,
              cex.axis = 1, cex.main = 1.2)
...
dev.off()
# Detrended transformed Owen’s plots
tiff(paste0(FigureFolder, "/Flanker/diagnosis_owenplot_mod_", str_split_i(as.character(model.Flanker$mu.formula), "~", 1)[2], ".tiff"),res=600, width=4000, height=3000)
dtop(model.Flanker)
dev.off()
...
```{r bootstrap}
# 1. Flanker
bootstrapdir <- paste0(interfileFolder, "/ABCD/bootstrap/Flanker")
...
quantile.median$age <- seq(min(Flanker_data$Age_year), max(Flanker_data$Age_year), length.out = n_points)
quantile.P2.5$age <- seq(min(Flanker_data$Age_year), max(Flanker_data$Age_year), length.out = n_points)
quantile.P97.5$age <- seq(min(Flanker_data$Age_year), max(Flanker_data$Age_year), length.out = n_points)

# plot
ggplot()+
  geom_point(data = Flanker_data, aes(x = Age_year, y = nihtbx_flanker_uncorrected), color = "grey", alpha = 0.3, width = 0, height = 0.1) +
  ...
  labs(x="Age (years)", y="Score", title="Bootstrap 95% CI of Flanker")+
  theme_classic()+
  scale_x_continuous(name="Age(years)", limits = c(8.8, 16),breaks = seq(9, 16, by = 1))+
...
ggsave(paste0(FigureFolder, "/Flanker/bootstrap_P50_95CI_Flanker_all.tiff"), width=9, height=7, units = "cm")
ggsave(paste0(FigureFolder, "/Flanker/bootstrap_P50_95CI_Flanker_all.pdf"), width=9, height=7, units = "cm")
